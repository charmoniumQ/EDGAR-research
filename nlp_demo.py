...
def extract_gutenberg(gutenberg_text):
    ...
    return gutenberg_text.split(start_marker)[1].split(end_marker)[0]
...
if __name__ == '__main__':
    import requests
    with time_code.ctx('download'):
        input_path = Path('../corpus2.txt')
        # if True:
        #     url = 'https://www.gutenberg.org/cache/epub/33/pg33.txt'
        #     raw_text = requests.get(url).text
        #     text = eliminate_wordwrap(extract_gutenberg(raw_text))
    ...
    with time_code.ctx('clean'):
        clean_text = text.replace('“', '"')

    with time_code.ctx('nlp'):
        paragraphs = text2paragraphs(clean_text)
        print(f'Paragraphs: {len(paragraphs)}')
        paragraphs = list(map(nlp, paragraphs))

    with time_code.ctx('entities and prhases'):
        paragraphs_entities = [
            [
                tuple([
                    token.lemma_
    ...
    with time_code.ctx('semantic words and stems'):
